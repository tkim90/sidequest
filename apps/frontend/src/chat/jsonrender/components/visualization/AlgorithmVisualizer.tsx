...
interface AlgorithmVisualizerScenarioProps {
  activeScenario: VisualizerScenario;
  baseDescription?: string;
  effectiveExampleIndex: number | null;
  markerSeed: string;
  onSelectExample: (index: number | null) => void;
  playbackDelayMs: number;
  selectableExamples: VisualizerScenario[];
}

function AlgorithmVisualizerScenario({
  activeScenario,
  baseDescription,
  ...
  playbackDelayMs,
  selectableExamples,
}: AlgorithmVisualizerScenarioProps) {
  ...
  if (totalSteps === 0) {
    return (
      <div className={`${surfaceClass} p-4`}>
        {activeScenario.title && <p className={titleClass}>{activeScenario.title}</p>}
  ...
  const PAD = 60;
  const rawMinX = Math.min(...nodes.map((node) => node.x));
  const rawMinY = Math.min(...nodes.map((node) => node.y));
  const rawMaxX = Math.max(...nodes.map((node) => node.x));
  const rawMaxY = Math.max(...nodes.map((node) => node.y));
              ...
              {nodes.length > 0 && (
                ...
                >
                  ...
                  {edges.map((edge, index) => {
                    ...
                    const isHighlighted = highlightEdgeSet.has(`${edge.from}->${edge.to}`);
                    const color = isHighlighted ? "var(--primary)" : "var(--muted-foreground)";
                    return (
                      <g key={`e-${index}`}>
                        <line
                          x1={x1}
                          ...
                          markerEnd={isHighlighted ? `url(#${arrowHighlightMarkerId})` : `url(#${arrowMarkerId})`}
                  ...
                  {nodes.map((node, index) => {
                    ...
                    const strokeColor = isHighlighted ? "var(--primary)" : "var(--border)";
                    const strokeWidth = isHighlighted ? 2.5 : 1.5;
                    const annotation = annotations[node.id];

                    return (
                      <g key={`node-${node.id}-${index}`} className="transition-all duration-150">
                        {shape === "circle" && (
                          <circle
                            cx={node.x}
                            cy={node.y}
